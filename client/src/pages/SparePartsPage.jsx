...
import { GET_ALL_APPROVED_SPARES } from '../graphql/Query';
...
const SparePartsPage = () => {
  ...
  const { loading: queryLoading } = useQuery(GET_ALL_APPROVED_SPARES, {
    onCompleted(data) {
      setData(data.getAllApprovedSpares);
    },
    onError(error) {
      console.error(error);
    },
      ...
      return data?.map((ad) => {
        return (
          <Card
            ...
            detail2={ad.make}
            detail3={ad.model}
            detail4={ad.avilability ? 'Available' : 'Unavailable'}
            key={ad.id}
          />
        );
      });
